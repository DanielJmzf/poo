...
public class MiPanel extends JPanel {
    ...
    private final Map<Componente, Point> offsetsComponentes = new HashMap<>();
    ...
    public MiPanel() {
        ...
        copiarItem.addActionListener(e -> {
            if (!componentesSeleccionados.isEmpty()) {
                circuito.copiarAlPortapeles(componentesSeleccionados);
        ...
        pegarItem.addActionListener(e -> {
            Point puntoPegado = getMousePosition();
            if (circuito != null) {
                circuito.pegarDesdePortapapeles(puntoPegado);
                repaint();
            }
        });
        
        menuContextual.add(copiarItem);
        menuContextual.add(pegarItem);
        
        addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (SwingUtilities.isRightMouseButton(e)) {                   
                    boolean hayComponentesParaCopiar = componenteSeleccionado != null || !componentesSeleccionados.isEmpty();
                    boolean hayComponentesParaPegar = circuito != null && !circuito.portapapeles.isEmpty();
                    
                    copiarItem.setEnabled(hayComponentesParaCopiar);
                    pegarItem.setEnabled(hayComponentesParaPegar);
...
private void dibujarConexion(Graphics2D g2d, Conector conector, boolean seleccionada) {
    ...
    private int getPinY(Pin pin) {
        ...
        if (pin.getComponente() instanceof Compuertas.CompuertaOR) {
            Compuertas.CompuertaOR or = (Compuertas.CompuertaOR)pin.getComponente();
            ...
            if (c != null) {
                ...
                for (Pin pin : c.getSalidas()) {
                    if (pin != null) {
                        ...
                        if (c instanceof Compuertas.CompuertaOR ) {
                            tamañoArea = 48;
        ...
        if (SwingUtilities.isLeftMouseButton(e)) {
            if (modoActual == Modo.SELECCION) {
                if (componenteSeleccionado != null && !componentesSeleccionados.isEmpty()) {
                    ...
                    componentesSeleccionados = circuito.seleccionarComponentesEnArea(rectanguloSeleccion);
                }
            }
            repaint();
        }
         if (!componentesSeleccionados.isEmpty() && puntoInicioArrastre != null) {
                ...
                for (Componente c : componentesSeleccionados) {
                    Point offset = offsetsComponentes.get(c);
                    if (offset != null) {
