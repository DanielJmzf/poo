...
public class Archivo {
    ...
    public static void guardarCircuitoComo(Circuito circuito, Main main) {
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(fileToSave))) {
                ...
                oos.writeObject(circuito);
                circuito.setModificado(false);
                JOptionPane.showMessageDialog(main, "Circuito guardado", "Éxito", JOptionPane.INFORMATION_MESSAGE);
            } catch (IOException excp) {
    ...
    public static Circuito cargarCircuito(Main main) {
        ...
        FileNameExtensionFilter filter = new FileNameExtensionFilter("Archivos de Circuito (*.cir)", "cir");
        fileChooser.setFileFilter(filter);
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fileToOpen))) {
                ...
                JOptionPane.showMessageDialog(main, "Error al cargar el circuito:\n" + excp.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
        return null;
    }catch (Exception excp) {
